    ...
    <div className="min-h-screen bg-brand-deep flex items-center justify-center p-6 relative overflow-hidden text-white">
      ...
      <div className="w-full max-w-md glass-panel p-8 rounded-2xl border border-brand-glass-border shadow-2xl relative z-10">
        <div className="text-center mb-8">
          ...
          <h2 className="text-2xl font-bold tracking-wider">WELCOME TO ATLANTIS</h2>
          <p className="text-xs text-brand-teal font-medium tracking-widest mt-1">SECURE ACCESS GATEWAY</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button
            type="submit"
            disabled={submitting}
            className="w-full py-3.5 mt-2 bg-gradient-to-r from-brand-teal to-brand-cyan text-brand-deep font-extrabold text-sm rounded-xl hover:shadow-lg hover:shadow-brand-teal/20 transition-all disabled:opacity-50 disabled:pointer-events-none"
          >
            {submitting ? 'VALIDATING SECURITY PARADIGMS...' : 'LOGIN TO OPERATING SYSTEM'}
          </button>
        </form>

        <div className="mt-6 text-center text-xs text-slate-400">
          <span>Don't have an academic account? </span>
          <Link to="/register" className="text-brand-teal hover:underline font-semibold">Initiate Registration</Link>
        </div>
      </div>
    </div>
  );
